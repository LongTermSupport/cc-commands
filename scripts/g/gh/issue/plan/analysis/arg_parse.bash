...
main() {
    ...
    if [ -z "$arguments" ]; then
        ...
        echo "Parsed issue from GitHub URL"
        echo "OPERATION_MODE=url"
        echo "PARSED_ISSUE_NUMBER=$ISSUE_NUM"
        echo "PARSED_REPOSITORY=$REPO"
        echo "PARSED_ISSUE_REFERENCE=$REPO#$ISSUE_NUM"
    elif [[ "$arguments" =~ ^([^/]+/[^/]+)#([0-9]+)$ ]]; then
        local REPO="${BASH_REMATCH[1]}"
        local ISSUE_NUM="${BASH_REMATCH[2]}"
        echo "MODE: \"REPO_ISSUE\""
        echo "ISSUE_NUMBER: \"$ISSUE_NUM\""
        echo "REPOSITORY: \"$REPO\""
        echo "Using repo#issue format"
        echo "OPERATION_MODE=repo_issue"
        echo "PARSED_ISSUE_NUMBER=$ISSUE_NUM"
        echo "PARSED_REPOSITORY=$REPO"
        echo "PARSED_ISSUE_REFERENCE=$REPO#$ISSUE_NUM"
    elif [[ "$arguments" =~ ^#?([0-9]+)$ ]]; then
        ...
        echo "MODE: \"NUMBER\""
        echo "ISSUE_NUMBER: \"$ISSUE_NUM\""
        echo "Using issue number directly (current repo)"
        echo "OPERATION_MODE=number"
        echo "PARSED_ISSUE_NUMBER=$ISSUE_NUM"
        echo "PARSED_ISSUE_REFERENCE=$ISSUE_NUM"
    else
        echo "MODE: \"INVALID\""
        echo "ISSUE_NUMBER: \"\""
        echo "Invalid argument format: $arguments"
        echo "Expected: GitHub URL, repo#issue, or issue number (#123)"
