...
export const GitHubErrorFactory = {
    ...
    authenticationError(context) {
        return new CommandError(new Error('GitHub authentication failed'), [
     ...
     */
    graphqlError(query, errors, context) {
        ...
        return new CommandError(new Error(`GitHub GraphQL query failed: ${errorMessages}`), [
     ...
     */
    permissionDeniedError(resource, requiredScope, context) {
        const scopes = requiredScope ? [requiredScope] : ['repo', 'read:project', 'read:org'];
        return new CommandError(new Error(`Permission denied: insufficient access to ${resource}`), [
     ...
     */
    projectNotFoundError(org, projectNumber) {
        return new CommandError(new Error(`Project #${projectNumber} not found in organization ${org}`), [
     ...
     */
    rateLimitError(resetTime, limit = 60, remaining = 0, context) {
        const now = new Date();
        const waitMinutes = Math.ceil((resetTime.getTime() - now.getTime()) / 60_000);
        const resetTimeLocal = resetTime.toLocaleTimeString();
        return new CommandError(new Error(`GitHub API rate limit exceeded (${remaining}/${limit} requests remaining)`), [
     ...
     */
    repositoryNotFoundError(owner, repo) {
        return new CommandError(new Error(`Repository ${owner}/${repo} not found or inaccessible`), [
