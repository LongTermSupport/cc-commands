...
import { hasGitHubAuth } from '../../../../utils/hasGitHubAuth';
/**
 ...
 */
export const executeProjectSummary = async (services, args, flags) => {
    ...
    if (mode === 'detect') {
        ...
        const repoDataArray = repoDataResults.filter((data) => data !== null);
        // Build complete project data structure
        const projectStructure = {
            aggregated: {
                mostActiveRepository: repoDataArray.sort((a, b) => b.activity.commits.total - a.activity.commits.total)[0]?.fullName,
    ...
    result.addAction('Orchestration complete', 'success', `Completed ${mode} mode execution`);
    return result;
};
