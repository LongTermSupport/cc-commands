 ...
 */
export class DataFileOrchestrationService {
    dataFileService;
    /**
    ...
    async execute(context) {
        ...
        try {
            ...
            if (dataType === 'project' && organization) {
                ...
                filepath = await this.dataFileService.saveProjectData(data, organization, projectNumber);
                result.addAction('Save project data', 'success', `Saved project data for ${organization}${projectNumber ? ` project #${projectNumber}` : ''}`);
            }
            else {
                // Save generic data
                if (!filename) {
                ...
                result.addAction('Save data file', 'success', `Saved data to ${filename}`);
            }
            ...
            result.setError(new CommandError(error, ['Ensure var/ directory is writable', 'Check disk space'], {
                dataType: dataType,
                ...(filename && { filename }),
