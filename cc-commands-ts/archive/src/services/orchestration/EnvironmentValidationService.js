...
import { EnvironmentValidationDTO } from '../../dto/EnvironmentValidationDTO';
...
export class EnvironmentValidationService {
    async execute(context) {
        ...
        try {
            const requiredTools = context.params['requiredTools'] || [];
            const requiredEnvVars = context.params['requiredEnvVars'] || [];
            ...
            // Add actions
            result.addAction('Environment validation', isValid ? 'success' : 'failed', isValid ? 'All requirements met' : `Missing: ${[...missingTools, ...missingEnvVars].join(', ')}`);
            ...
            if (!isValid) {
                ...
                result.setError(new CommandError(new Error('Environment validation failed'), [
