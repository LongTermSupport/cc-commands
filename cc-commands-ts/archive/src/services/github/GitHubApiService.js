...
export class GitHubApiService {
    ...
    async getProject(org, projectNumber) {
        try {
            const query = `
      ...
      `;
            const response = await this.octokit.graphql(query, { number: projectNumber, org });
    ...
    async getProjectRepositories(org, projectNumber) {
        try {
            const query = `
        query($org: String!, $number: Int!) {
          organization(login: $org) {
    ...
    async getRepositoryComments(owner, repo, since) {
        try {
            const comments = [];
            // Get issue comments
    ...
    async getRepositoryIssues(owner, repo, since) {
        try {
            const issues = [];
            // Get issues
    ...
    async listOrganizationProjects(org) {
      ...
      `;
            const response = await this.octokit.graphql(query, { org });
