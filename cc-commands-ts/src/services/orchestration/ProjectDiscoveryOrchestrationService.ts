...
import { ProjectDataService } from '../github/ProjectDataService.js'
 ...
 */
export class ProjectDiscoveryOrchestrationService implements IOrchestrationService {
  /**
  ...
  async execute(context: ServiceContext): Promise<LLMInfo> {
    ...
    const params = context.params as ProjectDiscoveryParams
    ...
    try {
      if (fetchItems && projectId) {
        ...
        const projects = await this.projectDataService.findOrganizationProjects(
          organization, 
