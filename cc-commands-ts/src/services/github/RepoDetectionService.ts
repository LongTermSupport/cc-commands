...
import { RepoDetectionResultDTO } from '../../dto/RepoDetectionResultDTO.js'
 ...
 */
export class RepoDetectionService implements IRepoDetectionService {
  constructor(
   ...
   */
  async detectFromDirectory(): Promise<RepoDetectionResultDTO> {
    const git = simpleGit()
   ...
   */
  async detectFromUrl(url: string): Promise<RepoDetectionResultDTO> {
    ...
    for (const pattern of patterns) {
      ...
      if (match && match[1] && match[2]) {
        return RepoDetectionResultDTO.fromURL(
          url,
          match[1],
          match[2].replace(/\.git$/, '')
        )
   ...
   */
  async detectRepository(owner: string, repo: string): Promise<RepositoryDataDTO> {
    if (!this.githubApi) {
